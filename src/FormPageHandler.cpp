...
void FormPageHandler::handleRequest(Poco::Net::HTTPServerRequest &request, Poco::Net::HTTPServerResponse &response) {
    ...
    //std::string html_template(form_html_start, form_html_end);
    std::ostringstream oss;
    //oss << html_template;//fin.rdbuf();
    std::string content(oss.str());
    //content = regex_replace( content, std::regex("@LINK1_URL@"), Mongoose::Utils::htmlEntities(request.get("link1", "") ));
    content = regex_replace( content, std::regex("@LINK1_URL@"), request.get("link1", "") );
    content = regex_replace( content, std::regex("@LINK2_URL@"), request.get("link2", "") );
    ...
    content = regex_replace( content, std::regex("@LINK_FOR_COPY_AND_PASTE@"), link );
